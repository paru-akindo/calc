...
if "defeated" not in st.session_state:
    st.session_state["defeated"] = []

# --- 生存している敵だけを抽出 ---
alive_df = df[~df["商会員名"].isin(st.session_state["defeated"])]

st.subheader("今回出てきた敵を入力")

def enemy_input(label):
    name = st.selectbox(f"{label}：商会員名", alive_df["商会員名"], key=f"name_{label}")
...
# --- 倒した敵を登録 ---
st.subheader("倒した敵を登録")
dead = st.selectbox("倒した敵", alive_df["商会員名"], key="dead_select")
if st.button("登録"):
    st.session_state["defeated"].append(dead)
