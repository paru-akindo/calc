...
def fetch_cfg():
    ...
    payload = resp.json()
    # jsonbin v3 の場合 record に入ることが多い
...
def parse_int_strict(s: str):
    ...
    if s == "":
        return None
    s = s.replace(",", "")
    s = s.translate(str.maketrans("０１２３４５６７８９－＋．，", "0123456789-+.,"))  # normalize
...
st.markdown(f"## {selected_port} の価格を入力（整数のみ、空欄は未入力で保存不可）")
# 現在の値を表示（あれば）
...
with col_ok:
    if st.button("この港の価格を保存"):
        ...
        if invalids:
            ...
            # 更新して保存（PRICES の形は port -> {item: price}）
            PRICES[selected_port] = new_row
            cfg["PRICES"] = PRICES
            try:
                resp = save_cfg(cfg)
                st.success(f"{selected_port} の価格を保存しました。jsonbin を更新しました。")
                st.write(resp)
